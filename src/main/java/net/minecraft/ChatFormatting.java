...
public enum ChatFormatting {
    ...
    private static final Map<String, ChatFormatting> FORMATTING_BY_NAME = Arrays.stream(values())
            .collect(Collectors.toMap(f -> cleanName(f.name), f -> f));
    ...
    public static ChatFormatting getByCode(char code) {
        char lower = Character.toString(code).toLowerCase(Locale.ROOT).charAt(0);
        for (ChatFormatting fmt : values()) {
            if (fmt.code == lower) {
                return fmt;
            }
    ...
    public static Collection<String> getNames(boolean includeColors, boolean includeFormats) {
        ...
        for (ChatFormatting fmt : values()) {
            if ((!fmt.isColor() || includeColors) && (!fmt.isFormat() || includeFormats)) {
